...
import { Users, Search, Scale, Shield, Zap, TrendingUp, Info, X, ChevronRight, Loader2, Star, Ghost, Clock, BarChart3, Trash2, Crown, Share2, Plus, BrainCircuit, CheckCircle2, AlertTriangle, CalendarDays, Timer } from 'lucide-react';
...
export default function FantasyHub() {
  ...
  const addToSquad = (player: any) => {
    ...
    if (isGk) targetRange = [10];
    else if (isDef) targetRange = [6, 7, 8, 9];
    else if (isFplMid) targetRange = [2, 3, 4, 5];
    else if (isFwd) targetRange = [0, 1];
  ...
  const upcomingGameweeks = useMemo(() => {
    ...
    return Object.keys(grouped).map(Number).sort((a, b) => a - b).slice(0, 3).map(gw => ({
        gw,
        matches: grouped[gw] 
      }));
  }, [fixturesData]);

  // 👇 1. لوجيك العداد التنازلي 👇
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const deadline = new Date(firstMatchDate).getTime();
  ...
  // 👇 2. لوجيك صعوبة المباريات (FDR) للاعب المحدد 👇
  const activePlayerFixtures = useMemo(() => {
    if (!activePlayer || !fixturesData?.matches) return [];
    const teamId = activePlayer.team?.id;
    if (!teamId) return [];

    return fixturesData.matches
      .filter((m: any) => m.homeTeam.id === teamId || m.awayTeam.id === teamId)
      .slice(0, 3)
      .map((m: any) => {
         const isHome = m.homeTeam.id === teamId;
         const opponent = isHome ? m.awayTeam : m.homeTeam;
         const oppName = opponent.tla || opponent.shortName || opponent.name.substring(0,3);
         
         // تقييم وهمي للصعوبة (FDR)
         const hardTeams = ['MCI', 'ARS', 'LIV'];
         const medTeams = ['CHE', 'TOT', 'MUN', 'NEW', 'AVL'];
         
         let fdrColor = "bg-emerald-500 text-white border-emerald-400"; // سهل
         if (hardTeams.includes(oppName.toUpperCase())) fdrColor = "bg-red-500 text-white border-red-400"; // صعب
         else if (medTeams.includes(oppName.toUpperCase())) fdrColor = "bg-zinc-600 text-white border-zinc-500"; // متوسط
  ...
  return (
    <div className="space-y-12 pb-20 max-w-6xl mx-auto px-4 sm:px-6"> {/* 📱 ظبطنا الـ Padding للموبايل */}
                ...
                <div className="p-2 grid grid-cols-1 gap-1">
                  {searchResults.map((player, idx) => (
                    <div key={player.id || `search-${idx}`} onClick={() => handleSelectForPreview(player)} className="flex items-center gap-3 px-3 py-2 md:px-4 md:py-3 text-left hover:bg-zinc-800/80 rounded-xl transition-all group cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0"> {/* 📱 Fix truncate on mobile */}
             ...
             {activePlayer ? (
               <motion.div key="active-card" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="rounded-3xl border border-indigo-500/30 bg-[#111113] p-6 md:p-8 shadow-2xl relative overflow-hidden ring-1 ring-indigo-500/20">
                 ...
                 <div className="relative">
                   ...
                   {/* 👇 2. إضافة صعوبة المباريات (FDR) هنا 👇 */}
      ...
      {/* 👇 1. قسم الجولات بالعداد التنازلي ⏳ 👇 */}
      ...
      <section className="bg-gradient-to-br from-[#111113] to-[#09090b] rounded-[2rem] md:rounded-[40px] p-6 md:p-12 border border-zinc-800 shadow-2xl relative overflow-hidden flex flex-col items-center">
