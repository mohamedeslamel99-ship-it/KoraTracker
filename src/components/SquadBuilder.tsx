...
export default function SquadBuilder({ squad, onRemovePlayer, totalBudget, captainId, viceCaptainId, setCaptain, setViceCaptain, onGenerateAI, isGeneratingAI, onSelectPlayer }: any) {
    ...
    <div className="flex flex-col items-center w-16 relative group mt-2">
      {player && (
        <div className="absolute -top-7 left-1/2 -translate-x-1/2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-30 bg-black/80 p-1 rounded-full border border-white/10 shadow-2xl">
          <button onClick={(e) => { e.stopPropagation(); setCaptain(player.id); }} className="w-5 h-5 bg-yellow-500 rounded-full text-black font-black text-[10px] flex items-center justify-center hover:scale-110">C</button>
          <button onClick={(e) => { e.stopPropagation(); setViceCaptain(player.id); }} className="w-5 h-5 bg-zinc-200 rounded-full text-black font-black text-[10px] flex items-center justify-center hover:scale-110">V</button>
      ...
      {/* خليناه قابل للضغط عشان يفتح الـ Preview */}
      <div 
      ...
      >
        {player ? (
           <div className="relative flex flex-col items-center">
              ...
              {captainId === player.id && <div className="absolute -top-2 -right-2 bg-yellow-400 text-black w-4 h-4 rounded-full flex items-center justify-center font-black text-[9px] border border-black z-20">C</div>}
              {viceCaptainId === player.id && captainId !== player.id && <div className="absolute -top-2 -right-2 bg-zinc-200 text-black w-4 h-4 rounded-full flex items-center justify-center font-black text-[9px] border border-black z-20">V</div>}
              {player.team?.crest && (
  ...
  return (
    <div className="flex flex-col items-center gap-6 w-full p-4">
      ...
      <div ref={squadRef} className="w-full max-w-[400px] bg-green-700 rounded-xl relative p-6 flex flex-col border-4 border-white shadow-2xl overflow-hidden" style={{ minHeight: '600px' }}>
        <div className="absolute top-2 left-2 text-white/40 text-[10px] font-bold tracking-widest z-10 flex items-center gap-1 uppercase">KoraTracker AI</div>
      ...
      <div className="flex gap-4 w-full max-w-[400px]">
         <button onClick={onGenerateAI} disabled={isGeneratingAI} className="flex-1 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 disabled:opacity-50 text-white font-black py-4 rounded-xl shadow-xl transition-all active:scale-95 uppercase text-[11px] tracking-widest flex items-center justify-center gap-2">{isGeneratingAI ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />}AI Analysis</button>
         <button onClick={downloadImage} className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white font-black py-4 rounded-xl shadow-xl transition-all active:scale-95 uppercase text-[11px] tracking-widest flex items-center justify-center gap-2">📸 Download</button>
      </div>
    </div>
  );
}