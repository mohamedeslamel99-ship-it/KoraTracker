...
export default function SquadBuilder({ 
  ...
  const PlayerSlot = ({ slotObj, index }: any) => {
    ...
    return (
      <div className="flex flex-col items-center w-[72px] md:w-[84px] relative group mt-2 z-20">
        
        {/* 🎛️ أزرار التحكم (بتظهر لما تقف بالماوس على اللاعب) */}
        {player && (
          <div className="absolute -top-8 left-1/2 -translate-x-1/2 flex gap-1 opacity-0 group-hover:opacity-100 transition-all duration-200 z-50 bg-black/90 p-1.5 rounded-full border border-white/20 shadow-2xl scale-95 group-hover:scale-100">
            <button onClick={(e) => { e.stopPropagation(); setCaptain(player.id); }} className="w-5 h-5 bg-yellow-500 hover:bg-yellow-400 rounded-full text-black font-black text-[10px] flex items-center justify-center transition-colors" title="Captain">C</button>
            <button onClick={(e) => { e.stopPropagation(); setViceCaptain(player.id); }} className="w-5 h-5 bg-zinc-300 hover:bg-white rounded-full text-black font-black text-[10px] flex items-center justify-center transition-colors" title="Vice Captain">V</button>
            <button onClick={(e) => { e.stopPropagation(); onSlotClick(index); }} className="w-5 h-5 bg-blue-500 hover:bg-blue-400 rounded-full text-white flex items-center justify-center transition-colors" title="Swap"><ArrowRightLeft size={10} strokeWidth={3} /></button>
            <button onClick={(e) => { e.stopPropagation(); onRemovePlayer(index, player.id); }} className="w-5 h-5 bg-red-500 hover:bg-red-400 rounded-full text-white flex items-center justify-center transition-colors" title="Remove"><X size={12} strokeWidth={3} /></button>
          </div>
        )}
        
        {/* 👕 التيشيرت (حالة الامتلاء وحالة الفراغ) */}
        <div 
          onClick={() => onSlotClick(index)}
          className={`relative flex flex-col items-center justify-center transition-all cursor-pointer mb-1.5
          ${isSwapping ? 'scale-110 drop-shadow-[0_0_15px_rgba(250,204,21,0.8)] animate-pulse' : 'hover:scale-105'}`}
        >
          {player ? (
            <>
              {/* التيشيرت مليان أبيض */}
              <Shirt size={44} strokeWidth={1} className="text-white fill-white drop-shadow-xl" onClick={(e) => { e.stopPropagation(); onSelectPlayer(player); }} />
              ...
              {/* لوجو الفريق */}
              {player.team?.crest && (
                <div className="absolute -bottom-1 -right-2 bg-white rounded-full p-[2px] shadow-sm flex items-center justify-center z-10 w-4 h-4 md:w-5 md:h-5">
            ...
            <>
              {/* التيشيرت شفاف في حالة الفراغ */}
              <Shirt size={44} strokeWidth={1} className="text-white/20 fill-white/5 drop-shadow-none" />
              {/* المركز مكتوب جوه التيشيرت */}
              <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-black text-[10px] text-white/50 tracking-widest">{role}</span>
            </>
          )}
        </div>

        {/* 📛 اسم اللاعب (تصميم فانتازي بريميوم) */}
        {player ? (
          <div 
            className="bg-[#37003c] text-white border border-[#00ff87] rounded text-[9px] md:text-[10px] font-black px-1.5 py-0.5 w-[70px] md:w-[80px] text-center line-clamp-2 leading-[1.1] shadow-lg cursor-pointer hover:bg-[#4a0052] transition-colors"
          ...
          <div className="bg-black/20 text-white/30 rounded text-[9px] font-black px-1.5 py-0.5 w-[60px] text-center border border-white/10 uppercase tracking-widest">
            Empty
          </div>
        )}
        
        {/* 💷 سعر اللاعب */}
        {player && <span className="text-[9px] font-black text-[#00ff87] mt-1 bg-black/60 px-2 py-0.5 rounded shadow-sm">£{player.price}m</span>}
      </div>
    );
  };

  // 📐 تصنيف التشكيلة ديناميكياً للرسم
  ...
  return (
    <div className="flex flex-col items-center gap-6 w-full p-4 relative z-0">
      ...
      <div ref={squadRef} className="w-full max-w-[400px] bg-green-700 rounded-xl relative p-6 flex flex-col border-4 border-white shadow-2xl overflow-hidden" style={{ minHeight: '600px' }}>
        ...
        <div className="flex justify-around mt-4 w-full px-2 z-20 min-h-[80px]">
           {pitchWithIndex.filter((item: any) => item.slot.role === 'FWD').map((item: any) => <PlayerSlot key={item.idx} slotObj={item.slot} index={item.idx} />)}
        </div>
        <div className="flex justify-around mt-8 w-full px-2 z-20 min-h-[80px]">
        ...
        {/* دكة البدلاء */}
        <div className="mt-auto pt-6 border-t border-white/20 bg-green-900/40 -mx-6 px-6 flex justify-around items-center h-32 relative z-20">
